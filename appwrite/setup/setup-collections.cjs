...
// `import` resolves to in a "type": "module" package. On some Node
// versions the ESM build's bundled undici request path throws a generic
// "fetch failed" even with valid credentials and a reachable endpoint;
...
// ─────────────────────────────────────────────────────────────────────────
// Every SDK method (databases.list, storage.listBuckets, createAttribute,
// etc.) ultimately funnels through Client.prototype.call. Some networks
// (flaky Wi-Fi, AV doing per-connection TLS inspection, occasional broken
// IPv6 routes) intermittently drop a single fresh connection attempt,
// which surfaces here as a generic "fetch failed" with no HTTP response
// at all — not a real auth/scope/API error. Retrying that exact call a
// few times with a short backoff is enough to ride out the blip without
// killing the whole run. Real API errors (bad key, missing scope, 4xx/5xx
// responses) are AppwriteException instances with a defined `code` and
// are NOT retried — only bare "fetch failed" TypeErrors are.
...
async function verifyPermissions({ databases, storage }, dbId) {
  ...
  try {
    await withRetry(() => databases.list(), { label: 'databases.list()' })
  ...
  try {
    await withRetry(() => storage.listBuckets(), { label: 'storage.listBuckets()' })
...
async function ensureDatabase({ databases }, dbId, dbName, report) {
  section('📦 Database')
  try {
    await withRetry(() => databases.get(dbId), { label: 'get database' })
...
function printSummary({ dbReport, colReports, bucketReport, validationPassed, rolledBack, hardFailure }) {
  console.log(`\n${c('bold', '═'.repeat(50))}`)
  ...
  if (rolledBack) {
    ...
    console.log(c('red', c('bold', '  Deployment crashed mid-run — see error above. Nothing further was applied.')))
